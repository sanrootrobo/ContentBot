...
class OllamaHumanizer:
    ...
    def _initialize_ollama(self):
        ...
        try:
            ...
            logger.info("Ollama 'openhermes' model initialized for humanizing.")
            return llm
        except Exception as e:
    ...
    try:
        ...
        if config.humanize:
            ...
            if not humanizer.llm:
                logger.error("Ollama Humanizer could not be initialized. The --humanize step will be skipped for all tasks.")
                humanizer = None
        ...
        def process_blog_task(blog_input: BlogInput):
            ...
            if result and result.get('success'):
                ...
                if humanizer:
                    ...
                    rephrased_content = humanizer.rephrase_content(original_content, blog_input.title, verbose=config.verbose)
                ...
                return f"❌ FAILED: '{blog_g_input.title}'. Reason: {error}"
