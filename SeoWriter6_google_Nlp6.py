...
# --- NEW: Dataclass for storing competitor data ---
@dataclass
...
class CompetitorAnalyzer:
    ...
    def _analyze_single_competitor(self, url: str) -> Optional[CompetitorData]:
        ...
        # A simple way to get main body text for word count and summary
...
class NewsAndDataResearcher:
    ...
    # MODIFIED: Accepts a shared GoogleSearchAPI instance
    def __init__(self, config: SEOBlogConfig, search_api: GoogleSearchAPI):
        self.config = config
        self.google_search = search_api
...
class SEOBlogWriter:
    # MODIFIED: Incorporates CompetitorAnalyzer into the workflow
    def __init__(self, config: SEOBlogConfig):
        self.config = config
        # Create a single GoogleSearchAPI instance to be shared
    ...
    def write_blog(self, blog_input: BlogInput) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info("[Phase 4/4] Generating final blog content...")
            llm = self._setup_llm()
    ...
    # MODIFIED: Prompt now includes detailed competitor analysis
...
"""
        ...
        return f"""
You are an expert SEO strategist and content creator. Your primary goal is to write a blog post that outranks the competition by being more comprehensive, up-to-date, and valuable.

**CRITICAL INSTRUCTIONS:**
1.  **Analyze the Competition:** Carefully review the `COMPETITOR ANALYSIS` section. Identify their structure, key points, word count, and any gaps in their content. Your article should be strategically better.
2.  **Improve and Expand:** Do not copy competitors. Synthesize their good ideas, then add unique insights, answer questions they missed, and provide a superior, more in-depth user experience.
3.  **Prioritize Freshness:** Use the `LATEST NEWS & FACTUAL RESEARCH` to ensure your content is current, data-driven, and trustworthy.
4.  **Output in Markdown:** The entire output must be valid Markdown. Structure the article logically with headings, lists, and tables if appropriate for the tone.
...
---
**YOUR TASK:**
Write the full blog post now. Use the competitor analysis to create a better outline and the research to provide fresh, factual details. Start with the most recent and exciting information. Ensure your article is significantly more thorough than the competition.
