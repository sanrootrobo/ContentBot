# main.py (fully integrated version with parallel processing removed from generation)
import os
import requests
import logging
import argparse
...
# Removed concurrent.futures as it's no longer needed for the main tasks
...
class OllamaHumanizer:
    ...
    def rephrase_content(self, original_content: str, title: str, verbose: bool = False) -> str:
        ...
        for part in parts_to_process:
            ...
            # Lock is not strictly necessary in sequential code, but harmless
            with Lock():
...
class SEOBlogConfig:
    ...
    # This setting is no longer used for the main loop, but kept for reference
...
class CompetitorAnalyzer:
    ...
    def analyze_competitor_content(self, competitors: List[Dict[str, Any]]) -> Dict[str, Any]:
        """MODIFIED: This method now runs sequentially."""
        if self.config.verbose: logger.info(f"Sequentially analyzing content from {len(competitors)} competitor pages...")
        successful_analyses = []
        # Removed ThreadPoolExecutor and replaced with a simple for loop
        for competitor in competitors:
            result = self._analyze_single_competitor(competitor['url'])
            if result['success']:
...
def parse_arguments():
    parser = argparse.ArgumentParser(description="Generate SEO-optimized blog posts sequentially.")
...
def main():
    ...
    try:
        ...
        # MODIFICATION: Execute tasks sequentially in a simple for loop
        logger.info(f"Found {len(blog_inputs)} blog(s) to generate sequentially.")
        for i, blog_input in enumerate(blog_inputs):
            logger.info(f"--- Starting job {i+1} of {len(blog_inputs)}: '{blog_input.title}' ---")
            try:
                message = process_blog_task(blog_input)
                print(f"\n{message}\n")
            except Exception as e:
                logger.error(f"A blog processing task failed unexpectedly for '{blog_input.title}': {e}", exc_info=True)
                print(f"\n❌ FAILED: '{blog_input.title}'. Reason: {e}\n")
