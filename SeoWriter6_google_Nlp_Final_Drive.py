...
# --- New Imports for Google Drive & Pandoc ---
import pypandoc
...
def authenticate_google_api():
    ...
    if not creds or not creds.valid:
        if not os.path.exists('credentials.json'):
            logger.error("Google Drive upload requires 'credentials.json'. Please obtain it from your Google Cloud project.")
            return None
...
def convert_md_to_odt(md_file_path: Path) -> Path:
    """Converts a markdown file to ODT format using pypandoc."""
...
def upload_to_drive(odt_file_path: Path, title: str, drive_service):
    """Uploads an ODT file to Google Drive, converting it to a Google Doc."""
    file_metadata = {
        'name': title,
...
def parse_arguments():
    ...
    parser.add_argument('--drive', action='store_true',
                       help='Upload the final blog post to Google Drive as a Google Doc.')
...
def save_enhanced_blog_to_file(blog_content: str, blog_input: BlogInput, metadata: Dict[str, Any]) -> Path:
    """Save blog content and metadata, returning the path to the markdown file."""
    ...
    try:
        ...
        for i, blog_input in enumerate(blog_inputs, 1):
            ...
            if result and result.get('success'):
                md_file_path = save_enhanced_blog_to_file(
                    result['blog_content'], 
                    blog_input, 
                    result.get('metadata', {})
                )
