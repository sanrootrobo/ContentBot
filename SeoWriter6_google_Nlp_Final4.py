...
class SEOBlogConfig:
    ...
    lightweight_model: str = "gemini-2.5-flash-lite"  # For queries and quick tasks
    heavy_model: str = "gemini-2.5-pro"  # For content generation and complex analysis
    timeout: int = 15
    max_retries: int = 3
...
class DualModelManager:
    """Manages both lightweight and heavy Gemini models."""
    
    def __init__(self, config: SEOBlogConfig):
        self.config = config
        api_key = APIKeyManager.get_api_key(config.api_key_file)
        
        # Initialize lightweight model for quick tasks
        self.lightweight_llm = ChatGoogleGenerativeAI(
            model=config.lightweight_model,
            google_api_key=api_key
        )
        
        # Initialize heavy model for complex tasks
        self.heavy_llm = ChatGoogleGenerativeAI(
            model=config.heavy_model,
            google_api_key=api_key
        )
        
        logger.info(f"Initialized dual models: {config.lightweight_model} (lightweight) + {config.heavy_model} (heavy)")
...
class QueryGenerator:
    ...
    def __init__(self, config: SEOBlogConfig, model_manager: DualModelManager):
        self.config = config
...
"""
    ...
    def generate_queries(self, blog_input: BlogInput) -> Dict[str, List[str]]:
        logger.info("Generating enhanced search queries with lightweight LLM...")
        prompt = self._build_prompt(blog_input)
        try:
            # Use lightweight model for query generation
            response = self.model_manager.lightweight_llm.invoke(prompt)
...
class EnhancedCompetitorAnalyzer:
    ...
    def __init__(self, config: SEOBlogConfig, search_api: GoogleSearchAPI, model_manager: DualModelManager):
        self.config = config
    ...
    def _identify_content_gaps_and_opportunities(self, content: str, title: str) -> Tuple[List[str], List[str]]:
        """Use lightweight LLM to identify content gaps and unique opportunities."""
        prompt = f"""
...
"""
    ...
    def _create_enhanced_summary(self, text: str, title: str) -> str:
        """Create a more insightful summary of competitor content using lightweight model."""
...
"""
        try:
            # Use lightweight model for summary generation
            response = self.model_manager.lightweight_llm.invoke(prompt)
...
class EnhancedNewsAndDataResearcher:
    ...
    def __init__(self, config: SEOBlogConfig, search_api: GoogleSearchAPI, model_manager: DualModelManager):
        self.config = config
        self.google_search = search_api
        self.query_generator = QueryGenerator(config, model_manager)
...
class HumanizedContentGenerator:
    """Specialized class for generating human-like, original content using heavy model."""
...
"""
    ...
    def generate_content(self, blog_input: BlogInput, research_data: Dict[str, List[str]], 
                        competitor_reports: List[EnhancedCompetitorData], 
                        insights: ContentInsights) -> str:
        """Generate content using the heavy model for complex content generation."""
        logger.info("Generating original, human-like content with heavy LLM (gemini-2.5-pro)...")
        prompt = self._build_humanized_prompt(blog_input, research_data, competitor_reports, insights)
        
        try:
            # Use heavy model for final content generation
            response = self.model_manager.heavy_llm.invoke(prompt)
...
class EnhancedSEOBlogWriter:
    """Enhanced blog writer with improved analysis and humanization using dual models."""
    
    def __init__(self, config: SEOBlogConfig):
        self.config = config
        self.model_manager = DualModelManager(config)
        self.google_search_api = GoogleSearchAPI(config)
        self.researcher = EnhancedNewsAndDataResearcher(config, self.google_search_api, self.model_manager)
        self.competitor_analyzer = EnhancedCompetitorAnalyzer(config, self.google_search_api, self.model_manager)
        self.content_generator = HumanizedContentGenerator(config, self.model_manager)
    ...
    def write_blog(self, blog_input: BlogInput) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info("[Phase 4/5] Generating Original, Human-like Content with Heavy Model...")
            blog_content = self.content_generator.generate_content(
                blog_input, research_data, competitor_reports, insights
            )
            
            logger.info("[Phase 5/5] Final Content Post-Processing...")
...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="Generate enhanced SEO-optimized blog posts with advanced competitor analysis and humanized content using dual Gemini models."
    )
    ...
    parser.add_argument('--lightweight-model', type=str, default='gemini-2.5-flash-lite',
                       help='Lightweight model for quick tasks (queries, summaries).')
    parser.add_argument('--heavy-model', type=str, default='gemini-2.5-pro',
                       help='Heavy model for complex tasks (content generation, deep analysis).')
...
def main():
    ...
    try:
        ...
        print(f"🤖 AI Models: {config.lightweight_model} (lightweight) + {config.heavy_model} (heavy)")
        ...
        for i, blog_input in enumerate(blog_inputs, 1):
            ...
            if result and result.get('success'):
                ...
                # Print success summary with model info
                metadata = result.get('metadata', {})
                models_used = metadata.get('models_used', {})
                print(f"\n✅ SUCCESS: Enhanced blog post '{blog_input.title}' completed!")
                print(f"   🤖 Models used: {models_used.get('lightweight_model', 'N/A')} + {models_used.get('heavy_model', 'N/A')}")
